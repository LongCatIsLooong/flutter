...
void main() {
  ...
  tearDown(() {
    TestWidgetsFlutterBinding.ensureInitialized()
      .defaultBinaryMessenger.setMockMethodCallHandler(SystemChannels.textInput, null);
    controller.dispose();
    focusNode.dispose();
  });
  ...
  testWidgets('Only focusable input fields are registered', (WidgetTester tester) async {
    // Regressoin test for https://github.com/flutter/flutter/issues/127803.
    EditableText buildEditableText() {
      return EditableText(
        controller: controller,
        backgroundCursorColor: Colors.grey,
        focusNode: focusNode,
    ...
    // This peculiar widget set up is a simple way to create a widget tree where
    // some obstructed render objects's `performLayout` are skipped.
    await tester.pumpWidget(
      MaterialApp(
        home: buildEditableText(),
        initialRoute: '/home',
        routes: <String, WidgetBuilder>{
          '/home': (BuildContext context) => buildEditableText(),
        },
      ),
    );
  ...
  testWidgets('Only focusable input fields are registered', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        ...
        initialRoute: '/home',
        routes: <String, WidgetBuilder>{
          '/home': (BuildContext context) => EditableText(
        controller: controller,
        backgroundCursorColor: Colors.grey,
        focusNode: FocusNode(debugLabel: 'Editable Text 2'),
        style: textStyle,
        cursorColor: cursorColor,
        selectionControls: materialTextSelectionControls,
      ),
  ...
  testWidgets('Scribble registration when focusability changes', (WidgetTester tester) async {
    EditableText buildEditableText() {
      return EditableText(
        controller: controller,
        backgroundCursorColor: Colors.grey,
    ...
    expect(TextInput.scribbleClients, isEmpty);
    ...
    expect(TextInput.scribbleClients, hasLength(1));
